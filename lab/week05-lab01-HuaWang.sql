...
ALTER TRIGGER trigPersonPwdInsert ON Person
AFTER INSERT
AS
BEGIN
	INSERT Password (PersonID, pwd)
	SELECT PersonID, 'pwd' FROM inserted
END
...
CREATE TRIGGER trigPaperInstanceInsert ON Paper
AFTER INSERT
AS
BEGIN
	INSERT PaperInstance (PaperID,SemesterID) SELECT PaperID, '2020S1' FROM inserted
	INSERT Enrolment (PaperID,SemesterID,PersonID) SELECT PaperID, '2020S1', '122' FROM inserted
END
...
CREATE TRIGGER trigWithdrawM ON Enrolment
AFTER DELETE
AS
BEGIN
	DELETE Withdrawn WHERE PaperID=(SELECT PaperID FROM inserted) AND SemesterID=(SELECT SemesterID FROM inserted WHERE LEFT(SemesterID,4)=DATEPART(YEAR,GETDATE())) AND PersonID=(SELECT PersonID FROM inserted)
	INSERT Withdrawn (PaperID,SemesterID,PersonID,WithdrawnDateTime) SELECT PaperID,SemesterID,PersonID,GETDATE() FROM inserted
END
...
CREATE TRIGGER trigWithdrawS ON Enrolment
FOR DELETE
AS
BEGIN
	IF (SELECT COUNT(*) FROM Withdrawn WHERE PaperID=(SELECT PaperID FROM inserted) AND SemesterID=(SELECT SemesterID FROM inserted WHERE LEFT(SemesterID,4)=DATEPART(YEAR,GETDATE())) AND PersonID=(SELECT PersonID FROM inserted))<1
	INSERT Withdrawn (PaperID,SemesterID,PersonID,WithdrawnDateTime) SELECT PaperID,SemesterID,PersonID,GETDATE() FROM inserted
END
...
ALTER TRIGGER trigPersonPwdUpdate ON Person
AFTER UPDATE
AS
BEGIN
	UPDATE Password SET pwd='npwd' WHERE PersonID=(SELECT PersonID FROM inserted)
END
